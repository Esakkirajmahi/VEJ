...
<h1>Login Form</h1>
			<div>
				<input type="text" name="mail" placeholder="Email-Id" required id="username" />
			</div>
			<div>
				<input type="password" name="passwd" placeholder="Password" required id="password" />
			</div>
			<div>
				<input type="submit" value="Submit" />
				<a href="#"><strong>Lost your password?</strong></a>
				<a href="CompanyRegister.jsp"><strong>Register</strong></a>
			</div>
				<c:set var="error_email" scope = "session" value = "0"></c:set>
					<c:set var="error_pass" scope ="session" value ="0"></c:set>
			 	<c:if test="${error3 == error_email }">
					<div><P>User not exists! Create Account</P></div>
					<% session.removeAttribute("error3"); %>
					</c:if>
					
					<c:if test="${ error4 == error_pass }">
